...
$releasePath = [System.IO.Path]::GetFullPath((Join-Path $appRoot $ReleaseDirectory))
...
Copy-Item -LiteralPath $iperfExecutable -Destination (Join-Path $releasePath 'iperf3.exe') -Force
Copy-Item -LiteralPath $cygwinRuntime -Destination (Join-Path $releasePath 'cygwin1.dll') -Force

$licensesDirectory = Join-Path $releasePath 'licenses'
...
& (Join-Path $releasePath 'iperf3.exe') --version
if ($LASTEXITCODE -ne 0) {
  throw "Bundled iperf3.exe smoke test failed with exit code $LASTEXITCODE."
}

Write-Host "Bundled iPerf $iperfVersion and Cygwin runtime $cygwinVersion in $releasePath"
