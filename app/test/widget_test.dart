...
void main() {
  ...
  testWidgets('desktop tool categories close the active tool route', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      NetToolsApp(state: state, initialLocation: '/tools'),
    );
    ...
    await tester.tap(find.text('远程与服务'));
    await tester.pumpAndSettle();

    expect(find.text('Ping'), findsNothing);
    expect(find.text('远程与服务'), findsOneWidget);
  });
}
