...
void main() {
  ...
  test('appends a six-byte SecureOn password when provided', () async {
    final listener = await RawDatagramSocket.bind(
      InternetAddress.loopbackIPv4,
      0,
    );
    ...
    try {
      ...
      final bytes = await packet.future.timeout(const Duration(seconds: 2));
      expect(bytes.length, 108);
      expect(bytes.sublist(102), [0xa1, 0xb2, 0xc3, 0xd4, 0xe5, 0xf6]);
    } finally {
      await subscription.cancel();
      listener.close();
    }
