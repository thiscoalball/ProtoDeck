...
class ToolDrafts extends Table {
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (migrator, from, to) async {
      if (from < 2) await migrator.createTable(toolDrafts);
    },
  );
  ...
  Future<ToolDraft?> getToolDraft(String scope) {
    return (select(
      toolDrafts,
    )..where((row) => row.scope.equals(scope))).getSingleOrNull();
  }

  Future<void> putToolDraft(ToolDraftsCompanion draft) {
    return into(toolDrafts).insertOnConflictUpdate(draft);
  }

  Future<void> deleteToolDraft(String scope) {
    return (delete(toolDrafts)..where((row) => row.scope.equals(scope))).go();
  }

  Future<void> clearToolDrafts() => delete(toolDrafts).go();
}
