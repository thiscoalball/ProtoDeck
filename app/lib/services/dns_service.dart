...
class DnsService {
  ...
  String _rdata(Uint8List bytes, int type, int offset, int length) {
    ...
    if (type == 43 && length >= 4) {
      return 'keyTag=${view.getUint16(offset)} '
          'algorithm=${bytes[offset + 2]} digestType=${bytes[offset + 3]} '
          'digest=${_hex(bytes, offset + 4, offset + length)}';
    }
    if (type == 44 && length >= 2) {
      return 'algorithm=${bytes[offset]} fingerprintType=${bytes[offset + 1]} '
          'fingerprint=${_hex(bytes, offset + 2, offset + length)}';
    }
    if (type == 48 && length >= 4) {
      return 'flags=${view.getUint16(offset)} protocol=${bytes[offset + 2]} '
          'algorithm=${bytes[offset + 3]} '
          'publicKey=${base64.encode(bytes.sublist(offset + 4, offset + length))}';
    }
    if (type == 52 && length >= 3) {
      return 'usage=${bytes[offset]} selector=${bytes[offset + 1]} '
          'matchingType=${bytes[offset + 2]} '
          'certificateAssociation=${_hex(bytes, offset + 3, offset + length)}';
    }
    if ((type == 64 || type == 65) && length >= 3) {
      return _serviceBindingData(bytes, offset, length);
    }
    return bytes
  ...
  String _serviceBindingData(Uint8List bytes, int offset, int length) {
    final view = ByteData.sublistView(bytes);
    final end = offset + length;
    final target = _readName(bytes, offset + 2);
    var cursor = target.nextOffset;
    if (cursor > end) throw const FormatException('SVCB TargetName 越界');
    final parameters = <String>[];
    while (cursor + 4 <= end) {
      final key = view.getUint16(cursor);
      final size = view.getUint16(cursor + 2);
      cursor += 4;
      if (cursor + size > end) throw const FormatException('SVCB 参数越界');
      parameters.add(_serviceBindingParameter(bytes, key, cursor, size));
      cursor += size;
    }
    return [
      'priority=${view.getUint16(offset)}',
      'target=${target.value.isEmpty ? '.' : target.value}',
      ...parameters,
    ].join(' ');
  }

  String _serviceBindingParameter(
    Uint8List bytes,
    int key,
    int offset,
    int length,
    ...
    switch (key) {
      case 0:
        if (length.isOdd) return 'mandatory=<invalid>';
        return 'mandatory=${[for (var cursor = offset; cursor < end; cursor += 2) _serviceBindingKey(view.getUint16(cursor))].join(',')}';
      case 1:
        final values = <String>[];
        var cursor = offset;
        while (cursor < end) {
          final size = bytes[cursor++];
          if (cursor + size > end) return 'alpn=<invalid>';
          values.add(ascii.decode(bytes.sublist(cursor, cursor + size)));
          cursor += size;
        }
        return 'alpn=${values.join(',')}';
      case 2:
      ...
      case 3:
        return length == 2
            ...
            : 'port=<invalid>';
      case 4:
        if (length % 4 != 0) return 'ipv4hint=<invalid>';
        return 'ipv4hint=${[for (var cursor = offset; cursor < end; cursor += 4) bytes.sublist(cursor, cursor + 4).join('.')].join(',')}';
      case 5:
        return 'ech=${base64.encode(bytes.sublist(offset, end))}';
      case 6:
        if (length % 16 != 0) return 'ipv6hint=<invalid>';
        return 'ipv6hint=${[for (var cursor = offset; cursor < end; cursor += 16) InternetAddress.fromRawAddress(Uint8List.fromList(bytes.sublist(cursor, cursor + 16))).address].join(',')}';
      case 7:
        return 'dohpath=${utf8.decode(bytes.sublist(offset, end), allowMalformed: true)}';
      default:
        return '${_serviceBindingKey(key)}=${_hex(bytes, offset, end)}';
    }
  }

  String _serviceBindingKey(int key) =>
      const {
        0: 'mandatory',
        1: 'alpn',
        2: 'no-default-alpn',
  ...
  String _hex(Uint8List bytes, int start, int end) => bytes
      .sublist(start, end)
      .map((byte) => byte.toRadixString(16).padLeft(2, '0'))
      .join()
      .toUpperCase();
