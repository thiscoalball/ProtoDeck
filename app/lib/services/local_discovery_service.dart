...
class LocalDiscoveryService {
  ...
  Future<List<SsdpDevice>> discoverSsdp({
    ...
    final normalizedTarget = searchTarget.trim();
    if (normalizedTarget.isEmpty ||
        normalizedTarget.length > 256 ||
        normalizedTarget.contains(RegExp(r'[\r\n]'))) {
      throw const FormatException('SSDP 搜索目标无效');
    }
  ...
  Future<List<MdnsRecord>> discoverMdns({
    Duration duration = const Duration(seconds: 4),
    String queryName = '_services._dns-sd._udp.local',
  }) async {
    final normalizedQuery = queryName.trim();
    if (normalizedQuery.isEmpty ||
        normalizedQuery.contains(RegExp(r'[\r\n]')) ||
        normalizedQuery
            .split('.')
            .any((label) => label.isEmpty || utf8.encode(label).length > 63)) {
      throw const FormatException('mDNS 查询名称无效');
    }
    await _acquireMulticastLock();
    late RawDatagramSocket socket;
    try {
