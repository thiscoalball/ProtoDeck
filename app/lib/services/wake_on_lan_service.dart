...
class WakeOnLanService {
  Future<int> send({
    ...
    final normalizedSecureOn = secureOn.replaceAll(RegExp(r'[^0-9A-Fa-f]'), '');
    if (secureOn.trim().isNotEmpty &&
        (normalizedSecureOn.length != 12 ||
            !RegExp(r'^[0-9A-Fa-f]{12}$').hasMatch(normalizedSecureOn))) {
      throw const FormatException('SecureOn 密码必须是 6 字节十六进制');
    }
    if (normalizedSecureOn.isNotEmpty) {
      packet.add([
        for (var index = 0; index < 12; index += 2)
          int.parse(normalizedSecureOn.substring(index, index + 2), radix: 16),
      ]);
    }
