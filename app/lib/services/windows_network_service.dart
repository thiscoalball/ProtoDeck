...
$tcp = @(Get-NetTCPConnection -ErrorAction SilentlyContinue | Select-Object -First 800 | ForEach-Object { $pidValue=[int]$_.OwningProcess; [pscustomobject]@{ protocol='TCP'; ipVersion=if ($_.RemoteAddress -like '*:*') { 6 } else { 4 }; localAddress=$_.LocalAddress; localPort=[int]$_.LocalPort; remoteAddress=$_.RemoteAddress; remotePort=[int]$_.RemotePort; state="$($_.State)"; uid=$null; pid=$pidValue; processName=$processNames[$pidValue] } })
$udp = @(Get-NetUDPEndpoint -ErrorAction SilentlyContinue | Select-Object -First 200 | ForEach-Object { $pidValue=[int]$_.OwningProcess; [pscustomobject]@{ protocol='UDP'; ipVersion=if ($_.LocalAddress -like '*:*') { 6 } else { 4 }; localAddress=$_.LocalAddress; localPort=[int]$_.LocalPort; remoteAddress=''; remotePort=0; state='Listening'; uid=$null; pid=$pidValue; processName=$processNames[$pidValue] } })
[pscustomobject]@{
