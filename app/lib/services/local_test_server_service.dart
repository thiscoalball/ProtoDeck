  ...
  Future<void> start({
    ...
    if (mode == LocalTestServerMode.http) {
      if (httpStatusCode < 100 || httpStatusCode > 599) {
        throw const FormatException('HTTP 状态码必须为 100～599');
      }
      try {
      ...
      for (final entry in httpHeaders.entries) {
        if (entry.key.trim().isEmpty ||
          ...
          throw const FormatException('自定义 HTTP 响应头格式无效');
        }
        if (const {
          'content-length',
          'transfer-encoding',
          ...
          throw FormatException('不允许手工设置响应头 ${entry.key}');
        }
      }
    }
  ...
  Future<void> _handleHttpRequest(
    ...
    try {
      ...
      _addEvent(
        ...
        peer: peer,
        summary:
            '${request.method} ${_safeRequestTarget(request.uri)} · $statusCode',
        detail: detail,
      );
    } on Object catch (error) {
      _addEvent(
