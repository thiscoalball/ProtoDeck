...
class DeveloperToolsService {
  Map<String, String> digestAll(String input, {String key = ''}) {
    final result = <String, String>{
      ...
      'SHA-224': digestText(input, 'sha224', key: key),
      'SHA-256': digestText(input, 'sha256', key: key),
      'SHA-384': digestText(input, 'sha384', key: key),
  ...
  Future<Map<String, String>> digestFile(File file) async {
    ...
    return {
      ...
      'SHA-224': await calculate(sha224),
      'SHA-256': await calculate(sha256),
      'SHA-384': await calculate(sha384),
      'SHA-512': await calculate(sha512),
    };
  }

  String decodeJwt(String input, {String secret = ''}) {
    ...
    final algorithm = header is Map ? header['alg']?.toString() ?? '' : '';
    final verification = _verifyJwtSignature(
    ...
    final output = <String>[
      ...
      parts[2],
      '',
      'Algorithm: ${algorithm.isEmpty ? '(missing)' : algorithm}',
    ...
    if (payload is Map) {
      ...
      for (final key in ['iat', 'nbf', 'exp']) {
        ...
        if (value is num) {
          ...
          if (key == 'exp') {
            output.add(
              'Token status: ${value.toInt() <= now ? 'EXPIRED' : 'VALID TIME WINDOW'}',
            );
          } else if (key == 'nbf' && value.toInt() > now) {
            output.add('Not-before status: NOT ACTIVE YET');
          }
        }
      }
      if (payload['iss'] != null) output.add('Issuer: ${payload['iss']}');
      if (payload['aud'] != null) output.add('Audience: ${payload['aud']}');
      if (payload['sub'] != null) output.add('Subject: ${payload['sub']}');
    }
    return output.join('\n');
  }

  String _verifyJwtSignature({
    ...
    if (hash == null) {
      return secret.isEmpty
          ? 'NOT VERIFIED (provide the matching public-key verifier externally)'
          : 'UNSUPPORTED ALGORITHM FOR SHARED-SECRET VERIFICATION';
    }
    if (secret.isEmpty) return 'NOT VERIFIED (enter an HMAC secret)';
  ...
  String generate(String type, {int length = 24}) {
    ...
    if (type == 'hex' || type == 'base64url') {
      if (length < 1 || length > 4096) {
        throw const FormatException('Token 字节数需为 1～4096');
      }
  ...
  String chmodConvert(String input) {
    ...
    if (RegExp(r'^[0-7]{3,4}$').hasMatch(text)) {
      final special = text.length == 4 ? int.parse(text[0]) : 0;
      final digits = text.substring(text.length - 3);
      ...
      if (special & 4 != 0) symbolic[2] = symbolic[2] == 'x' ? 's' : 'S';
      if (special & 2 != 0) symbolic[5] = symbolic[5] == 'x' ? 's' : 'S';
      if (special & 1 != 0) symbolic[8] = symbolic[8] == 'x' ? 't' : 'T';
      final octal = special == 0 ? digits : '$special$digits';
      return '八进制: $octal\n符号: ${symbolic.join()}\n'
          '特殊位: ${_specialPermissionLabel(special)}\nchmod $octal <file>';
    }
    final normalized = text.replaceAll(RegExp(r'^[bcdlps-]'), '');
    if (!RegExp(r'^[rwxSsTt-]{9}$').hasMatch(normalized) ||
        !RegExp(
          r'^[r-][w-][xSs-][r-][w-][xSs-][r-][w-][xTt-]$',
        ).hasMatch(normalized)) {
      throw const FormatException('请输入 755、4755、rwxr-xr-x 或 rwsr-xr-x');
    }
    final values = <int>[];
    for (var index = 0; index < 9; index += 3) {
      ...
      values.add(
        (part[0] == 'r' ? 4 : 0) +
            (part[1] == 'w' ? 2 : 0) +
            ('xst'.contains(part[2]) ? 1 : 0),
      );
    }
    final special =
        (RegExp(r'[sS]').hasMatch(normalized[2]) ? 4 : 0) |
        (RegExp(r'[sS]').hasMatch(normalized[5]) ? 2 : 0) |
        (RegExp(r'[tT]').hasMatch(normalized[8]) ? 1 : 0);
    final octal = '${special == 0 ? '' : special}${values.join()}';
    return '符号: $normalized\n八进制: $octal\n'
        '特殊位: ${_specialPermissionLabel(special)}\nchmod $octal <file>';
  }

  String _specialPermissionLabel(int value) {
    if (value == 0) return '无';
    return [
      if (value & 4 != 0) 'setuid',
      if (value & 2 != 0) 'setgid',
  ...
  String unicodeConvert(String input, {bool decode = false}) {
    if (!decode) {
      return input.runes.map((rune) {
        if (rune <= 0x7f) return String.fromCharCode(rune);
        if (rune <= 0xffff) {
        ...
        return '\\u${high.toRadixString(16).padLeft(4, '0')}'
            '\\u${low.toRadixString(16).padLeft(4, '0')}';
      }).join();
    }
    return input.replaceAllMapped(
      RegExp(
        r'\\u\{([0-9a-fA-F]{1,6})\}|\\U([0-9a-fA-F]{8})|\\u([0-9a-fA-F]{4})',
      ),
      (match) {
    ...
    return input.replaceAllMapped(
      ...
      (match) {
        ...
        if (value == null ||
          ...
          throw FormatException('HTML 实体码点无效：${match.group(0)}');
        }
        return String.fromCharCode(value);
      },
    );
  ...
  String compression(String input, {bool decode = false}) {
    if (decode) {
      ...
      if (bytes.length > 16 * 1024 * 1024) {
        throw const FormatException('压缩数据不能超过 16 MiB');
      }
      final decoded = gzip.decode(bytes);
      if (decoded.length > 32 * 1024 * 1024) {
        throw const FormatException('解压后数据超过 32 MiB 安全上限');
      }
      return utf8.decode(decoded, allowMalformed: false);
    }
    final source = utf8.encode(input);
    if (source.length > 16 * 1024 * 1024) {
      throw const FormatException('待压缩文本不能超过 16 MiB');
    }
    final compressed = gzip.encode(source);
    final encoded = base64.encode(compressed);
    final ratio = compressed.length / max(1, source.length) * 100;
    final saved = source.length - compressed.length;
    return '原始: ${source.length} B\nGzip: ${compressed.length} B\n'
        '压缩比: ${ratio.toStringAsFixed(1)}%\n'
        '节省: $saved B (${(100 - ratio).toStringAsFixed(1)}%)\n'
        'CRC32: ${digestText(input, 'crc32')}\n'
        'Base64 长度: ${encoded.length}\n\n$encoded';
  }
  ...
  String queryJson(String input, String path) {
    ...
    while (expression.isNotEmpty) {
      RegExpMatch? match;
      if ((match = RegExp(r'^\.\.([A-Za-z0-9_-]+)').firstMatch(expression)) !=
          null) {
        final key = match!.group(1)!;
        nodes = [for (final node in nodes) ..._recursiveJsonValues(node, key)];
      } else if ((match = RegExp(
            r'^\.([A-Za-z0-9_-]+)',
          ).firstMatch(expression)) !=
          null) {
        final key = match!.group(1)!;
        nodes = _jsonProperty(nodes, key);
      } else if ((match = RegExp(
            r'''^\[['"]((?:\\.|[^'"])*)['"]\]''',
          ).firstMatch(expression)) !=
          null) {
        final key = match!
            .group(1)!
            .replaceAllMapped(RegExp(r'\\(.)'), (value) => value.group(1)!);
        nodes = _jsonProperty(nodes, key);
      } else if ((match = RegExp(r'^\[\*\]').firstMatch(expression)) != null) {
        nodes = [
          for (final node in nodes)
            if (node is List) ...node else if (node is Map) ...node.values,
        ];
      } else if ((match = RegExp(r'^\[(-?\d+)\]').firstMatch(expression)) !=
          null) {
        final requested = int.parse(match!.group(1)!);
        final selected = <Object?>[];
        for (final node in nodes) {
          if (node is! List) continue;
          final index = requested < 0 ? node.length + requested : requested;
          if (index >= 0 && index < node.length) selected.add(node[index]);
        }
        nodes = selected;
      } else if ((match = RegExp(
            r'^\[(-?\d*):(-?\d*)\]',
        ...
        for (final node in nodes) {
          if (node is! List) continue;
          int boundary(String? raw, int fallback) {
            if (raw == null || raw.isEmpty) return fallback;
            final value = int.parse(raw);
            return value < 0 ? node.length + value : value;
          }

          final start = boundary(match!.group(1), 0).clamp(0, node.length);
          final end = boundary(
            match.group(2),
            node.length,
          ).clamp(start, node.length);
          selected.addAll(node.sublist(start, end));
        }
        nodes = selected;
      } else if ((match = RegExp(
        ...
        final expected = _jsonPathLiteral(match.group(3)!);
        nodes = [
          for (final node in nodes)
            if (node is List)
              for (final item in node)
                if (item is Map &&
                    _jsonPathCompare(item[key], expected, operator))
                  item,
        ];
      } else {
        throw FormatException('无法识别 JSONPath：$expression');
      }
      expression = expression.substring(match!.group(0)!.length);
    }
    final Object? result = nodes.length == 1 ? nodes.single : nodes;
    return result is String
        ? result
        : const JsonEncoder.withIndent('  ').convert(result);
  }

  List<Object?> _jsonProperty(List<Object?> nodes, String key) => [
    for (final node in nodes)
  ...
  Iterable<Object?> _recursiveJsonValues(Object? value, String key) sync* {
    if (value is Map) {
      if (value.containsKey(key)) yield value[key];
      for (final child in value.values) {
        yield* _recursiveJsonValues(child, key);
      }
  ...
  Object? _jsonPathLiteral(String raw) {
    if (raw.startsWith("'")) {
  ...
  bool _jsonPathCompare(Object? actual, Object? expected, String operator) {
  ...
  String nextCronRuns(String expression, {int count = 10}) {
    if (count < 1 || count > 100) {
      throw const FormatException('Cron 结果数量应为 1～100');
    }
    final fields = _expandCronMacro(expression.trim()).split(RegExp(r'\s+'));
    if (fields.length != 5) {
    ...
    for (
      var tries = 0;
      tries < 60 * 24 * 366 * 8 && results.length < count;
      tries++
    ) {
      final weekday = cursor.weekday == DateTime.sunday ? 0 : cursor.weekday;
      final dayOfMonth = _cronMatch(fields[2], cursor.day, 1, 31);
      final dayOfWeek = _cronMatch(fields[4], weekday, 0, 7, sunday: true);
      // POSIX/Vixie cron treats restricted day-of-month and day-of-week as OR.
      final dayMatches = fields[2] == '*'
          ? dayOfWeek
          : fields[4] == '*'
          ? dayOfMonth
      ...
      if (_cronMatch(fields[0], cursor.minute, 0, 59) &&
          ...
          cursor.day <= DateTime(cursor.year, cursor.month + 1, 0).day) {
        results.add(cursor);
      }
      cursor = cursor.add(const Duration(minutes: 1));
    }
    if (results.isEmpty) throw const FormatException('八年内没有匹配时间，请检查表达式');
    return [
      '表达式: ${fields.join(' ')}',
  ...
  String _expandCronMacro(String expression) =>
      switch (expression.toLowerCase()) {
        ...
        '@daily' || '@midnight' => '0 0 * * *',
        '@hourly' => '0 * * * *',
        '@reboot' => throw const FormatException('@reboot 取决于调度器启动，无法计算下次时间'),
        _ => _normalizeCronNames(expression),
      };

  String _normalizeCronNames(String expression) {
    ...
    String replaceNames(String value, Map<String, String> names) {
    ...
    fields[3] = replaceNames(fields[3], months);
    fields[4] = replaceNames(fields[4], weekdays);
    return fields.join(' ');
  }
  ...
  bool _cronMatch(
    ...
    for (final item in field.split(',')) {
      ...
      if (stepParts.length > 2 || stepParts.any((part) => part.isEmpty)) {
        throw FormatException('Cron 步长错误：$item');
      }
      ...
      final candidates = sunday && value == 0 ? const [0, 7] : [value];
  ...
  String parseUserAgent(String input) {
    ...
    final os = value.contains('Android')
        ...
        ? _windowsVersion(first(RegExp(r'Windows NT ([0-9.]+)')))
        : value.contains('Mac OS X')
        ? 'macOS ${first(RegExp(r'Mac OS X ([0-9_]+)')).replaceAll('_', '.')}'
        : value.contains('CrOS')
        ? 'ChromeOS ${first(RegExp(r'CrOS [^ ]+ ([^\) ]+)'))}'
        : value.contains('Linux')
        ? 'Linux'
        : '未知';
    final browser = value.contains('Edg/') || value.contains('EdgiOS/')
        ? 'Edge ${first(RegExp(r'(?:Edg|EdgiOS)/([^ ]+)'))}'
        : value.contains('OPR/')
        ? 'Opera ${first(RegExp(r'OPR/([^ ]+)'))}'
        : value.contains('SamsungBrowser/')
        ? 'Samsung Internet ${first(RegExp(r'SamsungBrowser/([^ ]+)'))}'
        : value.contains('CriOS/')
        ? 'Chrome iOS ${first(RegExp(r'CriOS/([^ ]+)'))}'
        : value.contains('FxiOS/')
        ? 'Firefox iOS ${first(RegExp(r'FxiOS/([^ ]+)'))}'
    ...
    final mobile = value.contains('Mobile') || value.contains('iPhone');
    final device = tablet
        ? '平板设备'
        : mobile
        ? '移动设备'
        : '桌面/其他设备';
    final engine = value.contains('Gecko/') && value.contains('Firefox/')
        ? 'Gecko'
        ...
        : '未知';
    final architecture =
        RegExp(r'arm64|aarch64', caseSensitive: false).hasMatch(value)
        ? 'ARM64'
        : RegExp(
            r'x86_64|x64|Win64|WOW64',
            caseSensitive: false,
          ).hasMatch(value)
        ? 'x64'
        : RegExp(r'i[3-6]86|x86', caseSensitive: false).hasMatch(value)
        ? 'x86'
        : '未知';
    final model = value.contains('Android')
        ? _androidModel(value)
        : value.contains('iPad')
        ? 'iPad'
    ...
    final webView =
        value.contains('; wv') ||
        (value.contains('Version/4.0') && value.contains('Android'));
    final bot = RegExp(
      r'bot|crawler|spider|slurp|bingpreview|headless',
      caseSensitive: false,
    ).hasMatch(value);
    return '浏览器: $browser\n渲染引擎: $engine\n系统: $os\n'
        '设备类型: $device\n设备型号: $model\n架构: $architecture\n'
        'WebView: ${webView ? '是' : '否'}\n'
        'Bot/Crawler: ${bot ? '可能是' : '否'}';
  }

  String _windowsVersion(String ntVersion) => switch (ntVersion) {
    '10.0' => 'Windows 10/11',
    '6.3' => 'Windows 8.1',
    '6.2' => 'Windows 8',
    '6.1' => 'Windows 7',
  ...
  String _androidModel(String userAgent) {
    final remainder = RegExp(
      r'Android [^;\)]+;([^\)]+)',
    ).firstMatch(userAgent)?.group(1);
    if (remainder == null) return '未知';
    for (final raw in remainder.split(';')) {
      final value = raw.trim().replaceFirst(RegExp(r'\s+Build/.+$'), '');
      if (value.isEmpty ||
          value == 'wv' ||
          RegExp(
            r'^[a-z]{2}(?:[-_][a-z]{2})?$',
  ...
  Map<String, String> bitCalculate({
    ...
    final b = right == null || right.trim().isEmpty
        ...
        : _parseNumber(right);
    final mask = (BigInt.one << width) - BigInt.one;
    final normalizedA = a & mask;
    final shift = b.toInt();
    if (shift < 0) throw const FormatException('移位位数不能为负数');
    final rotation = shift % width;
    final result =
        switch (operation) {
          'AND' => normalizedA & b,
          'OR' => normalizedA | b,
          'XOR' => normalizedA ^ b,
          'NOT' => ~normalizedA,
          'SHL' => normalizedA << shift,
          'SHR' => normalizedA >> shift,
          'ROL' =>
            rotation == 0
                ? normalizedA
                : (normalizedA << rotation) |
                      (normalizedA >> (width - rotation)),
          'ROR' =>
            rotation == 0
                ? normalizedA
                : (normalizedA >> rotation) |
        ...
        mask;
    final signed = result & (BigInt.one << (width - 1)) == BigInt.zero
        ? result
        : result - (BigInt.one << width);
    final bytes = List<int>.generate(
      width ~/ 8,
      (index) =>
          ((result >> ((width ~/ 8 - index - 1) * 8)) & BigInt.from(0xff))
              .toInt(),
    );
    final byteText = bytes
        ...
        .toUpperCase();
    return {
      '无符号十进制': result.toString(),
      '有符号十进制（补码）': signed.toString(),
      '十六进制':
          '0x${result.toRadixString(16).padLeft(width ~/ 4, '0').toUpperCase()}',
      '二进制': result.toRadixString(2).padLeft(width, '0'),
      '大端字节': byteText,
      '小端字节': byteText.split(' ').reversed.join(' '),
    };
  }
