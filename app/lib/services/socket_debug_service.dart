...
class SocketDebugService {
  SocketDebugService._();

  static final SocketDebugService instance = SocketDebugService._();

  final _events = StreamController<SocketDebugEvent>.broadcast();
  final _history = <SocketDebugEvent>[];
  ...
  int? _udpTargetPort;
  String _status = '未启动';
  SocketDebugProtocol? _protocol;
  SocketDebugRole? _role;

  Stream<SocketDebugEvent> get events => _events.stream;
  List<SocketDebugEvent> get history => List.unmodifiable(_history);
  bool get running => _tcpServer != null || _tcpClient != null || _udp != null;
  int get peerCount => _tcpPeers.length + (_tcpClient == null ? 0 : 1);
  List<String> get peers => List.unmodifiable([
    if (_tcpClient case final client?) _tcpPeer(client),
    ..._tcpPeers.map(_tcpPeer),
  ]);
  int get receivedBytes => _history
      .where((event) => event.direction == 'RX')
      .fold(0, (sum, event) => sum + event.bytes.length);
  int get sentBytes => _history
      .where((event) => event.direction == 'TX')
      .fold(0, (sum, event) => sum + event.bytes.length);
  String get status => _status;
  SocketDebugProtocol? get protocol => _protocol;
  SocketDebugRole? get role => _role;
  ...
  Future<String> start({
    ...
    if (protocol == SocketDebugProtocol.tcp) {
      if (role == SocketDebugRole.server) {
        ...
        _role = role;
        return _status =
            'TCP Server ${_tcpServer!.address.address}:${_tcpServer!.port}';
      }
      ...
      _protocol = protocol;
      _role = role;
      return _status = 'TCP Client ${socket.address.address}:${socket.port}';
    }
    ...
    _role = role;
    return _status =
        'UDP ${role == SocketDebugRole.server ? 'Server' : 'Client'} '
  ...
  Future<void> send(
    ...
    if (_tcpServer != null) {
      ...
      final targets = peer == null || peer.isEmpty
          ? List<Socket>.from(_tcpPeers)
          : _tcpPeers.where((socket) => _tcpPeer(socket) == peer).toList();
      if (targets.isEmpty) throw StateError('选择的 TCP 客户端已断开');
      for (final socket in targets) {
        socket.add(bytes);
        await socket.flush();
        _emit('TX', _tcpPeer(socket), bytes);
      }
      return;
    }
    final udp = _udp;
