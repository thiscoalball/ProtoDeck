...
class ApiWorkbenchService {
  ...
  /// Imports the common, portable subset of a cURL command without invoking a
  /// shell. Unsupported flags are ignored with their following scalar value,
  /// while file expansion and command substitution are never evaluated.
  static CurlImportResult parseCurl(String command) {
    final tokens = _shellTokens(command.trim());
    if (tokens.isEmpty || tokens.first.toLowerCase() != 'curl') {
      throw const FormatException('输入内容不是 cURL 命令');
    }
    var method = '';
    var url = '';
    var body = '';
    ...
    for (var index = 1; index < tokens.length; index++) {
      ...
      String next() {
        if (++index >= tokens.length) {
          throw FormatException('cURL 参数缺少值：$token');
        }
        return tokens[index];
      }
      ...
      if (token == '-X' || token == '--request') {
        ...
        final header = token.substring(9);
        final separator = header.indexOf(':');
        if (separator > 0) {
          headers[header.substring(0, separator).trim()] =
              header.substring(separator + 1).trim();
        }
      } else if ({'-d', '--data', '--data-raw', '--data-binary', '--data-urlencode'}.contains(token)) {
        final value = next();
        ...
        url = token.substring(6);
      } else if (!token.startsWith('-') &&
          (token.startsWith('http://') || token.startsWith('https://'))) {
        url = token;
      } else if ({'-u', '--user', '-A', '--user-agent', '-e', '--referer', '--connect-timeout', '--max-time'}.contains(token)) {
        next();
      }
    }
    if (url.isEmpty) throw const FormatException('cURL 命令中没有 HTTP URL');
    if (method.isEmpty) method = body.isEmpty ? 'GET' : 'POST';
  ...
  static List<String> _shellTokens(String source) {
    final result = <String>[];
    ...
    if (quote != null) throw const FormatException('cURL 命令的引号没有闭合');
    flush();
    return result;
  }
}
