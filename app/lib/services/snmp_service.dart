...
class SnmpService {
  ...
  Stream<List<SnmpVariable>> walkV3({
    ...
    while (!_cancelled && rows.length < maxRows) {
      final response = await requestV3(
        host: host,
        credentials: credentials,
        oids: [current],
        port: port,
        operation: SnmpOperation.getNext,
        timeout: timeout,
      );
      if (response.variables.isEmpty) break;
      final next = response.variables.first;
      final normalized = _normalizeOid(next.oid);
      if (!(normalized == _normalizeOid(rootOid) ||
          normalized.startsWith(root)))
        break;
      if (_compareOid(normalized, current) <= 0)
        throw StateError('设备返回了不递增的 OID，Walk 已停止');
      rows.add(next);
      current = normalized;
