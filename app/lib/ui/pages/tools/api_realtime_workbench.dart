...
import '../../../services/api_realtime_session_registry.dart';
...
class _ApiRealtimeWorkbenchState extends State<ApiRealtimeWorkbench> {
  final _sessionRegistry = ApiRealtimeSessionRegistry.instance;
  late final WebSocketDebugSession _webSocket = _sessionRegistry.webSocket;
  late final MqttDebugSession _mqtt = _sessionRegistry.mqtt;
  ...
  void initState() {
    super.initState();
    _environment = Map.of(widget.environment ?? const {});
    _messages.addAll(_sessionRegistry.historyFor(widget.protocol));
    _connected = _sessionRegistry.isConnected(widget.protocol);
    _subscriptions.add(
      _sessionRegistry.messages.listen((message) {
        if (_messageProtocol(message) == widget.protocol) _onMessage(message);
      }),
    );
    _subscriptions.add(
      _sessionRegistry.connectionChanges.listen((protocol) {
        if (protocol == widget.protocol && mounted) {
          setState(() => _connected = _sessionRegistry.isConnected(protocol));
        }
      }),
    );
  ...
  void didUpdateWidget(covariant ApiRealtimeWorkbench oldWidget) {
    ...
    if (oldWidget.protocol != widget.protocol) {
      setState(() {
        _messages
          ..clear()
          ..addAll(_sessionRegistry.historyFor(widget.protocol));
        _connected = _sessionRegistry.isConnected(widget.protocol);
        _configTab = 0;
        _applyProtocolDefaults();
      });
  ...
  int _messageProtocol(RealtimeMessage message) => switch (message.protocol) {
    'WebSocket' => 1,
    'SSE' => 2,
    'MQTT' => 3,
    _ => 0,
  ...
  Future<void> _connect() async {
    ...
    try {
      if (widget.protocol == 1)
        await _sessionRegistry.connectWebSocket(
          _connectionUrl(),
          headers: _handshakeHeaders(),
        );
      if (widget.protocol == 2)
        await _sessionRegistry.connectSse(
          _connectionUrl(),
          headers: _handshakeHeaders(),
          method: _sseMethod,
          body: _sseBody.text,
      ...
      if (widget.protocol == 3) {
        await _sessionRegistry.connectMqtt(
  ...
  Future<void> _disconnect() async {
    await _sessionRegistry.disconnect(widget.protocol);
    if (mounted)
      setState(() {
        _connected = false;
  ...
  Uint8List _messageBytes() {
    final source = ApiWorkbenchService.substitute(_message.text, _environment)
  ...
  void _applyState(
    ...
    void restoreRows(String key, List<ApiFieldRow> target) {
      try {
        ...
        // Ignore malformed legacy secure values.
      }
    }
  ...
  Future<void> _loadState() async {
    ...
    final selectedIndex = widget.initialTemplateId == null
        ? -1
        : _connectionTemplates.indexWhere(
            (item) => item['id']?.toString() == widget.initialTemplateId,
          );
    if (selectedIndex >= 0) await _loadConnectionTemplate(selectedIndex);
  }
          ...
          content: Column(
            ...
            children: [
              ...
              CheckboxListTile(
                ...
                subtitle: const LocalizedText('密码、Cookie 与敏感 Header 将写入系统安全存储'),
              ),
            ],
          ),
          actions: [
