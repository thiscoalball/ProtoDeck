...
class QrGeneratorPage extends StatefulWidget {
  const QrGeneratorPage({super.key, required this.appState});
...
class _QrGeneratorPageState extends State<QrGeneratorPage> {
  ...
  @override
  void dispose() {
    if (_draftLoaded) unawaited(_drafts.save('tool.qr', _draftValue()));
    _drafts.dispose();
    _primary.dispose();
    _secondary.dispose();
    _tertiary.dispose();
  ...
  Map<String, Object?> _draftValue() => {
    ...
    // A Wi-Fi password is deliberately never stored as a normal tool draft.
    if (_type != QrContentType.wifi) 'secondary': _secondary.text,
    if (_type != QrContentType.wifi) 'tertiary': _tertiary.text,
    'wifiEncryption': _wifiEncryption,
    'wifiHidden': _wifiHidden,
    'foreground': _foreground.text,
    'background': _background.text,
    'size': _size,
  };

  Future<void> _restoreDraft() async {
    final draft = await _drafts.load('tool.qr');
    if (!mounted) return;
    if (draft != null) {
      _type = QrContentType.values.firstWhere(
        (value) => value.name == draft.payload['type'],
        orElse: () => _type,
      );
      _primary.text = draft.payload['primary']?.toString() ?? _primary.text;
      if (_type != QrContentType.wifi) {
        _secondary.text = draft.payload['secondary']?.toString() ?? '';
        _tertiary.text = draft.payload['tertiary']?.toString() ?? '';
      }
      _wifiEncryption =
          draft.payload['wifiEncryption']?.toString() ?? _wifiEncryption;
      _wifiHidden = draft.payload['wifiHidden'] == true;
      _foreground.text =
          draft.payload['foreground']?.toString() ?? _foreground.text;
      _background.text =
          draft.payload['background']?.toString() ?? _background.text;
      _size = (draft.payload['size'] as num?)?.toInt() ?? _size;
      _generate();
    }
    _draftLoaded = true;
  }
  ...
  void _saveDraft() {
    if (_draftLoaded) _drafts.scheduleSave('tool.qr', _draftValue());
  }
}
