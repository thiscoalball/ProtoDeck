...
class _IperfPageState extends State<IperfPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) unawaited(_restoreDraftAndAttach());
    });
  }
  ...
  void dispose() {
    ...
    _eventTimer?.cancel();
    if (_draftLoaded) {
      unawaited(_drafts.save('tool.iperf', _draftValue()));
    }
    _drafts.dispose();
    _controller.dispose();
                    ...
                    children: [
                      ...
                      IconButton(
                        onPressed: () => Clipboard.setData(
                          ClipboardData(text: context.tr(_output)),
                        ),
  ...
  Future<void> _start() async {
    ...
    try {
      await _native.startForegroundTask(
        ...
        _mode == IperfMode.server
            ? context.tr('等待客户端连接，可从通知或页面停止')
            : _controller.text,
      );
  ...
  Future<void> _restoreDraftAndAttach() async {
    final draft = await _drafts.load('tool.iperf');
    if (!mounted) return;
    final payload = draft?.payload;
    if (payload != null) {
      _mode = IperfMode.values.firstWhere(
        (value) => value.name == payload['mode'],
        orElse: () => _mode,
      );
      _controller.text = payload['command']?.toString() ?? _controller.text;
    }
    _draftLoaded = true;
    final running = await _native.isIperfRunning();
    if (!mounted) return;
    if (!running) {
      setState(() {});
      return;
    ...
    _elapsedTimer = Timer.periodic(const Duration(seconds: 1), (_) async {
      final stillRunning = await _native.isIperfRunning();
      if (!mounted) return;
      if (stillRunning) {
  ...
  void _saveDraft() {
    if (_draftLoaded) {
      _drafts.scheduleSave('tool.iperf', _draftValue());
    }
  }
}
