...
class BluetoothDebugPage extends StatefulWidget {
  const BluetoothDebugPage({super.key, required this.appState});
...
class _BluetoothDebugPageState extends State<BluetoothDebugPage> {
  ...
  static final _devices = <String, Map<Object?, Object?>>{};
  static final _rssiHistory = <String, List<_RssiPoint>>{};
  static final _notifyEnabled = <String>{};
  static final _logs = <Map<Object?, Object?>>[];
  static List<Map<Object?, Object?>> _services = const [];
  ...
  bool get _scanning => _section == 0 ? _bleScanning : _classicScanning;
  ...
  static String _connectionState = '未连接';
  ...
  void dispose() {
    ...
    if (_bleScanning) unawaited(_service.stopBleScan());
    if (_classicScanning) unawaited(_service.stopClassicScan());
    _bleScanning = false;
    _classicScanning = false;
    // Active client/server sessions deliberately survive page navigation.
    // They are released only by the explicit disconnect/stop actions.
    if (_draftLoaded) {
      unawaited(_drafts.save('tool.bluetooth_debug', _draftValue()));
    }
    _drafts.dispose();
    for (final value in [
      _classicUuid,
  ...
  Future<void> _initialize() async {
    try {
      ...
      if (Platform.isWindows || Platform.isLinux) {
        final paired = status['supported'] == true
            ? await _service.bondedDevices()
            : const <Map<Object?, Object?>>[];
        if (!mounted) return;
        setState(() {
          _status = status;
          _scanIssue = status['supported'] == true
              ? null
              : _desktopBluetoothReason(status);
          for (final value in paired) {
            final address = value['address'] as String?;
            if (address != null) _devices[address] = {...value, 'bonded': true};
          }
        });
        return;
      }
      ...
      final scanRequest = apiLevel >= 31
          ? requested[Permission.bluetoothScan]
          : requested[Permission.locationWhenInUse];
      final scanDenied =
          scanRequest?.isDenied == true ||
          scanRequest?.isPermanentlyDenied == true;
      ...
      setState(() {
        _status = status;
        _scanIssue = !scanGranted || scanDenied
      ...
      children: [
        ...
        ...[
          ...
          SegmentedButton<int>(
            ...
            selected: {_section},
            onSelectionChanged: (value) => _switchSection(value.first),
          ),
          const SizedBox(height: 12),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: LocalizedText('连接状态：$_connectionState')),
                if (_connectedAddress != null)
                  OutlinedButton.icon(
                    onPressed: _disconnectClient,
                    icon: const Icon(Icons.link_off_rounded, size: 18),
                    label: const LocalizedText('断开'),
                  ),
              ],
            ),
            if (_scanIssue != null) ...[
  ...
  String _desktopBluetoothReason(Map<Object?, Object?> status) =>
      switch (status['errorCode']?.toString()) {
        'bluetooth.adapterMissing' => '未检测到可用的蓝牙适配器',
        'bluetooth.bluezUnavailable' => 'BlueZ 服务不可用，请安装并启动系统蓝牙服务',
        'bluetooth.windowsBackendUnavailable' => '当前 Windows 构建尚未包含 WinRT 蓝牙后端',
        _ => '蓝牙能力当前不可用',
      };
        ...
        builder: (context, controller) => ListView(
          ...
          children: [
            ...
            if (_section == 0) ...[
              ...
              _detailSection(
                '经典蓝牙信息',
                '设备类型: ${_classicType(device['deviceType'] as int? ?? (device['type'] is int ? device['type'] as int : null))}\n'
  ...
  Future<void> _toggleScan() async {
    final scanSection = _section;
    final wasScanning = scanSection == 0 ? _bleScanning : _classicScanning;
    try {
      if (wasScanning) {
        _scanWatchdog?.cancel();
        scanSection == 0
        ...
        _scanWatchdog = Timer(const Duration(seconds: 6), () {
          ...
          final stillScanning = scanSection == 0
              ? _bleScanning
              : _classicScanning;
          if (mounted && stillScanning && !receivedAdvertisement) {
            setState(() {
              _scanIssue = scanSection == 0
      ...
      if (mounted) {
        setState(() => _setScanning(scanSection, !wasScanning));
      }
    } on Object catch (error) {
      _message('$error');
    }
  ...
  void _setScanning(int section, bool value) {
    if (section == 0) {
      _bleScanning = value;
    } else {
      _classicScanning = value;
    }
  }

  Future<void> _switchSection(int nextSection) async {
    if (nextSection == _section) return;
    final previousSection = _section;
    _scanWatchdog?.cancel();
    try {
      if (previousSection == 0 && _bleScanning) {
        await _service.stopBleScan();
      } else if (previousSection == 1 && _classicScanning) {
        await _service.stopClassicScan();
      }
    } on Object catch (error) {
      _message('$error');
    ...
    if (!mounted) return;
    setState(() {
      _setScanning(previousSection, false);
  ...
  Future<void> _connect(Map<Object?, Object?> device) async {
    ...
    try {
      if (Platform.isAndroid && (_status['apiLevel'] as int? ?? 31) >= 31) {
        final permission = await Permission.bluetoothConnect.request();
        if (!permission.isGranted) {
          _message('连接设备需要授予附近设备连接权限');
          return;
        }
      }
      if (_section == 0) {
    ...
    setState(() {
      ...
      if (type == 'bleDevice' || type == 'classicDevice') {
        ...
        if (event['state'] == 'finished' || event['state'] == 'stopped') {
          _setScanning(type == 'bleScan' ? 0 : 1, false);
        }
      }
      if (type != 'bleDevice' && type != 'classicDevice') {
  ...
  Map<String, Object?> _draftValue() => {
    'section': _section,
    'server': _server,
    'classicUuid': _classicUuid.text,
    'serviceUuid': _serviceId.text,
    'characteristicUuid': _characteristicId.text,
    'payload': _payload.text,
    'search': _search.text,
    'hex': _hex,
    'hideUnnamed': _hideUnnamed,
  ...
  Future<void> _restoreDraft() async {
    final draft = await _drafts.load('tool.bluetooth_debug');
    if (!mounted) return;
    final payload = draft?.payload;
    if (payload != null) {
      final section = (payload['section'] as num?)?.toInt() ?? 0;
      _section = Platform.isWindows ? 0 : section.clamp(0, 1);
      _server = !Platform.isWindows && payload['server'] == true;
      _classicUuid.text =
          payload['classicUuid']?.toString() ?? _classicUuid.text;
      _serviceId.text = payload['serviceUuid']?.toString() ?? _serviceId.text;
      _characteristicId.text =
          payload['characteristicUuid']?.toString() ?? _characteristicId.text;
      _payload.text = payload['payload']?.toString() ?? _payload.text;
      _search.text = payload['search']?.toString() ?? '';
      _hex = payload['hex'] == true;
      _hideUnnamed = payload['hideUnnamed'] == true;
      _onlyConnectable = payload['onlyConnectable'] == true;
      _minRssi = ((payload['minRssi'] as num?)?.toInt() ?? -100).clamp(
        -100,
        -30,
      );
      _sort = const {'rssi', 'name', 'seen'}.contains(payload['sort'])
  ...
  void _saveDraft() {
    if (_draftLoaded) {
      _drafts.scheduleSave('tool.bluetooth_debug', _draftValue());
    }
  }
}
