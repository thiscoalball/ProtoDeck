...
class _EventRow {
  const _EventRow(this.snapshot, this.change);
...
class _NetworkEventMonitorPageState extends State<NetworkEventMonitorPage> {
  final _service = NetworkEventService();
  ...
  void _start() {
    ...
    _subscription = _service.watch().listen(
      (snapshot) {
        ...
        _current = snapshot;
        final changes = snapshot.changesFrom(previous);
        for (final change in changes) {
          if (_events.length >= 2000) _events.removeAt(0);
          _events.add(_EventRow(snapshot, change));
        }
