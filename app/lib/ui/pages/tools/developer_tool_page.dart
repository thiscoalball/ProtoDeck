...
class DeveloperToolPage extends StatefulWidget {
  const DeveloperToolPage({super.key, required this.mode});
  final String mode;
...
class _DeveloperToolPageState extends State<DeveloperToolPage> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: LocalizedText(_title)),
    body: ListView(
      ...
      children: [
        ...
        if (widget.mode == 'bits')
          DropdownButtonFormField<String>(
            initialValue: _choice,
            items: const ['AND', 'OR', 'XOR', 'NOT', 'SHL', 'SHR']
          ...
          Row(
            children: [
              Expanded(
                child: TextField(
                  controller: _second,
                  enabled: _choice == 'password',
                  keyboardType: TextInputType.number,
                  decoration: const InputDecoration(
                    label: LocalizedText('密码长度（默认 24）'),
                  ),
                ),
              ),
              const SizedBox(width: 10),
  ...
  void _run() {
    try {
      ...
      final result = switch (widget.mode) {
        ...
        'hash' => _runTextDigest(),
        'jwt' => _service.decodeJwt(_input.text),
  ...
  List<_InputPreset> get _presets => switch (widget.mode) {
    'base64' => const [
      _InputPreset('UTF-8 文本', 'ProtoDeck 网络工具箱'),
      _InputPreset('JSON', '{"name":"ProtoDeck","ok":true}'),
    ],
    'url_codec' => const [
      _InputPreset('查询参数', '关键词=网络 工具&平台=Android'),
      _InputPreset('完整网址', 'https://example.com/search?q=网络 工具'),
    ],
    'chmod' => const [
    ...
    'unicode' => const [_InputPreset('中英文', 'ProtoDeck 网络工具箱')],
    'html_entity' => const [
    ...
    'compression' => const [
      _InputPreset('重复文本', 'ProtoDeck ProtoDeck ProtoDeck 网络工具箱'),
    ],
    'diff' => const [
      _InputPreset(
        '配置差异',
...
const _regexTemplates = <_RegexTemplate>[
  ...
  _RegexTemplate('中国大陆手机号', r'(?<!\d)1[3-9]\d{9}(?!\d)', '联系电话：13800138000'),
  _RegexTemplate(
    '十六进制颜色',
