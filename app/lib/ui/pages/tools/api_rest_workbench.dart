...
class ApiRestWorkbench extends StatefulWidget {
  const ApiRestWorkbench({super.key});
...
class _ApiRestWorkbenchState extends State<ApiRestWorkbench> {
      ...
      children: [
        ...
        SingleChildScrollView(
          ...
          child: SegmentedButton<int>(
            showSelectedIcon: false,
            segments: const [
              ButtonSegment(value: 0, label: LocalizedText('Pretty')),
              ButtonSegment(value: 1, label: LocalizedText('Raw')),
              ButtonSegment(value: 2, label: LocalizedText('Headers')),
              ButtonSegment(value: 3, label: LocalizedText('Cookies')),
              ButtonSegment(value: 4, label: LocalizedText('实际请求')),
            ],
  ...
  Future<void> _loadState() async {
    ...
    try {
      ...
      final secrets = await _workspaceStore.loadSecrets('rest_draft');
      if (!mounted) return;
      setState(() {
        _environment = env.map((key, value) => MapEntry(key, '$value'));
        _templates = templates;
        if (draft != null) _applyState(draft, secrets: secrets);
        _stateLoaded = true;
  ...
  Future<void> _saveTemplate({Map<String, Object?>? overwrite}) async {
    ...
    if (accepted == true && name.text.trim().isNotEmpty) {
      ...
      await _workspaceStore.saveSecrets('rest_template_$id', _secretSnapshot());
      if (mounted) {
        setState(() {});
  ...
  Future<void> _saveDraft({String? fingerprint, bool force = false}) async {
    ...
    await Future.wait([
      ...
      _workspaceStore.saveSecrets('rest_draft', _secretSnapshot()),
    ]);
