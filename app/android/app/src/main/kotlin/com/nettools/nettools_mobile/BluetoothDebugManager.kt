...
class BluetoothDebugManager(private val context: Context) {
    ...
    fun startClassicScan(): Boolean {
        ...
        val started = value.startDiscovery()
        check(started) { "系统拒绝启动经典蓝牙发现，请稍后重试或重启蓝牙" }
    ...
    fun stopClassicScan() {
        ...
        emit("classicScan", mapOf("state" to "stopped"))
    }
    ...
    private fun deviceMap(device: BluetoothDevice): Map<String, Any?> = mapOf(
        ...
        // Keep this distinct from the event envelope's `type` field. Using the
        // same key overwrote `bleDevice` / `classicDevice` and made valid scan
        // callbacks invisible to Flutter.
        "deviceType" to (if (hasConnectPermission()) device.type else BluetoothDevice.DEVICE_TYPE_UNKNOWN),
    )

    private fun emit(type: String, data: Map<String, Any?>) {
        // Envelope fields win over payload fields so a device/service property
        // can never turn a scan event into an unrecognised event type.
        events.add(data + mapOf("type" to type, "time" to System.currentTimeMillis()))
    ...
    private fun requireScan() {
        check(hasScanPermission()) { "缺少附近设备扫描权限" }
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S) {
            val location = context.getSystemService(Context.LOCATION_SERVICE) as LocationManager
            check(location.isLocationEnabled) { "Android 10/11 蓝牙扫描需要开启系统定位服务" }
        }
    }
